				...
				<ul class="navbar-nav mx-auto text-center">
					...
					<li class="{$class|cat:" dropdown"}"><a class="nav-link dropdown-toggle" href="index.php?page={$link}" id="navbarDropdown" data-toggle="dropdown">{$navItem.text}</a>
						<div class="dropdown-menu" aria-labelledby="navbarDropdown">
							{foreach $navItem.submenu as $subLink => $subItem} <a class="dropdown-item" href="index.php?page={$link}&subPage={$subLink}">{$subItem.text}</a>
							{/foreach}
						</div></li> {else}
					<li class="{$class}"><a class="nav-link" href="index.php?page={$link}">{$navItem.text}</a></li> {/if} {/if} {/foreach}
				</ul>
				<span class="navbar-text  bg-secondary text-white px-3 text-right font-weight-bold">##DAY## {$currentDay}, {$dayTime}{if $money !== false} |
					{$money|number_format:0:",":"."}{/if}</span>
			</div>
		</nav>
	</header>
